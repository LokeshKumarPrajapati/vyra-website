...
class ProjectManager:
    def __init__(self, workspace_root: str):
        ...
        # Use a permanent project for conversations instead of temp
        self.current_project = "Conversations"

        # Ensure projects root exists
        if not self.projects_dir.exists():
            self.projects_dir.mkdir(parents=True)

        # Clear temp project on startup if it exists (for backwards compatibility)
        temp_path = self.projects_dir / "temp"
        if temp_path.exists():
            print("[ProjectManager] Clearing old temp project...")
            shutil.rmtree(temp_path)

        # Create permanent Conversations project if it doesn't exist
        conversations_path = self.projects_dir / "Conversations"
        if not conversations_path.exists():
            print(
                "[ProjectManager] Creating permanent 'Conversations' project for chat history...")
            self.create_project("Conversations")
    ...
    def save_cad_artifact(self, source_path: str, prompt: str):
        ...
        if not os.path.exists(source_path):
            print(
                f"[ProjectManager] [ERR] Source file not found: {source_path}")
            return None

        # Create a filename based on timestamp and prompt
        timestamp = int(time.time())
        # Brief sanitization of prompt for filename
        safe_prompt = "".join([c for c in prompt if c.isalnum() or c in (
            ' ', '-', '_')])[:30].strip().replace(" ", "_")
    ...
    def get_project_context(self, max_file_size: int = 10000) -> str:
        ...
        text_extensions = {'.txt', '.py', '.js', '.jsx', '.ts',
                           '.tsx', '.json', '.md', '.html', '.css', '.jsonl'}
