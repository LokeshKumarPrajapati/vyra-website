        ...
        try:
            ...
            for attempt in range(max_retries):
                ...
                code_match = re.search(
                    r'```python(.*?)```', raw_content, re.DOTALL)
                if code_match:
                    code = code_match.group(1).strip()
                else:
                    # Fallback: assume entire text is code if no blocks, or fail
                    print(
                        "[Cadagent DEBUG] [WARN] No ```python block found. Trying heuristic...")
                    if "import build123d" in raw_content:
                        code = raw_content
                    else:
                        print(
                            "[Cadagent DEBUG] [ERR] Could not extract python code.")
                        return None
                ...
                with open(script_path, "w") as f:
                    ...
                    code_with_path = code.replace(
                        "output.stl", safe_output_path)
                    f.write(code_with_path)

                print(
                    f"[Cadagent DEBUG] [EXEC] Running local script: {script_path}")
                ...
                if proc.returncode != 0:
                    ...
                    print(
                        f"[Cadagent DEBUG] [ERR] Script Execution Failed:\n{error_msg}")
...
"""
                ...
                if os.path.exists(output_stl):
                    ...
                    print(
                        f"[Cadagent DEBUG] [ERR] '{output_stl}' was not generated.")
    ...
    async def iterate_prototype(self, prompt: str, output_dir: Optional[str] = None):
        ...
        if os.path.exists(script_path):
            ...
            print(
                "[Cadagent DEBUG] [WARN] No existing script found. Falling back to fresh generation.")
...
"""
            ...
            for attempt in range(max_retries):
                print(
                    f"[Cadagent DEBUG] Iteration Attempt {attempt + 1}/{max_retries}")
